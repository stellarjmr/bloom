...
`bm clean` moves safe, rebuildable cache and temporary files to the macOS Trash. It is conservative by design: protected system paths, Trash, iCloud/Mobile Documents, Apple Notes, Apple Mail, Keychains, Safari/Cookies, browser history/cookies, LaunchAgents/LaunchDaemons, Messages, Reminders, Photos libraries, Finder metadata, active application/developer-tool caches, live SQLite families, and other high-value data are skipped or rejected instead of cleaned. Hard safety entries remain protected even when loading an older or replacement-style whitelist, and activity is checked again immediately before a Trash move. Open-file checks require `lsof` visibility that includes root-owned processes, and incomplete downloads are rebound after sizing and at the action boundary. If a process, SQLite, or open-file safety probe times out or is interrupted, Bloom preserves the current target and stops the remaining clean run rather than continuing with partial evidence.

The interactive flow lives at menu item 5. Use `bm clean --dry-run` to preview targets first, and use Config → Clean or `bm clean --whitelist` to choose additional whitelist entries that Bloom should protect. Sizes use physical disk occupancy; each directory probe is time-bounded, and Bloom skips an item whose complete size cannot be measured.
...
`bm uninstall` removes a macOS `.app` bundle plus the leftovers most apps drop into `~/Library` (Application Support, Caches, Containers, Group Containers, HTTPStorages, WebKit, Logs, Saved Application State, Application Scripts, Preferences, ByHost preferences, LaunchAgents, and Cookies). The app list uses the bundle's localized Finder-facing name when available, while cleanup matching continues to use only the bundle basename and identifiers. Bloom also scans the app for embedded helper bundles (XPC services, app extensions, and login-item helpers) and cleans their leftovers by exact bundle ID, while skipping protected identifiers so removal stays scoped to the app's own data. The exact shared developer roots `~/.local`, `~/.config`, and `~/.cache` are rejected both during discovery and at the final Trash boundary.
...
Before any uninstall side effect, Bloom binds the selected bundle's on-disk identity and scans its application roots again for other `.app` copies with the same Bundle ID. This includes iOS/iPadOS-style `Wrapper/*.app/Info.plist` identities; an unreadable or malformed candidate makes the scan incomplete. For a non-Homebrew app with a sibling copy, Bloom moves only the explicitly selected bundle to Trash and preserves shared leftovers, processes, login items, and Dock entries. For a Homebrew cask, Bloom refuses the uninstall instead of running the required `--zap` against data that a sibling may still use. An incomplete sibling scan is treated the same conservative way, and the scan is repeated at the action boundary.
...
- Each row shows the app name, physical on-disk size, and last-used time (`kMDItemLastUsedDate`, with bundle mtime as a fallback). CJK and fullwidth names align by display width.
- The summary lists every removed path under each app (`✓` removed, `·` would-remove).

Per-app cleanup runs in this order so brew can detect and clean its own metadata and cask-defined zap leftovers:

1. Bind the selected bundle identity, scan for same-Bundle-ID copies, and verify Homebrew ownership; repeat those checks immediately before acting.
2. Quit the app and unload its `LaunchAgents` when no sibling copy may depend on them. Before sending either `TERM` or `KILL`, Bloom revalidates the process start identity and executable inside the selected bundle so a reused PID cannot redirect the signal.
3. Detect the Homebrew cask via the `<prefix>/Caskroom/<token>/<version>` layout (resolved-symlink → bundle-name search → `brew list`/`info` fallback) and run `brew uninstall --cask --force --zap <token>`, then verify with `brew list --cask`. Homebrew zap may remove extra cask-defined support or configuration files; Bloom then moves any remaining curated leftovers to Trash.
4. Move the app bundle to the current user's Trash. Root-owned bundles, including many Mac App Store apps, may request terminal administrator authentication; Bloom still moves the bundle to Trash rather than permanently deleting it.
5. Remove the matching `~/Library` entries only after the app bundle is gone, then remove its Login Item and unregister the selected LaunchServices path.
