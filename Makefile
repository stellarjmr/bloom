...
release:
	...
	$(RELEASE_GO_ENV) GOOS=darwin GOARCH=arm64 go build -ldflags="$(LDFLAGS)" -o $(RELEASE_TMP)/bm-darwin-arm64/bm-core ./cmd/bloom
	$(RELEASE_GO_ENV) GOOS=darwin GOARCH=amd64 go build -ldflags="$(LDFLAGS)" -o $(RELEASE_TMP)/bm-darwin-amd64/bm-core ./cmd/bloom
